    ...
    Msg: Clone + std::fmt::Debug + Send + 'static,
{
    ...
    fn draw(&self, _state: &(), _cursor: mouse::Cursor, _bounds: Rectangle) -> VideoPrim {
        VideoPrim { frame: self.frame.clone() }
    }
}
...
impl shader::Primitive for VideoPrim {
    ...
    fn prepare(
        ...
        let frame = match &self.frame { Some(f) => f, None => return };
        ...
        let rebuild = pipeline.inner.as_ref()
    ...
    fn draw(
        ...
        let Some(inner) = &pipeline.inner else { return false };
...
impl GpuInner {
    fn new(device: &wgpu::Device, format: wgpu::TextureFormat, w: u32, h: u32) -> Self {
        ...
        Self { texture, bind_group, pipeline, last_gen: 0, tex_w: w, tex_h: h }
    }
}
